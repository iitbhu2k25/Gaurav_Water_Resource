...
export const MapProvider: React.FC<MapProviderProps> = ({ children }) => {
  ...
  const highlightSelectedFeatures = useCallback((layerType: string, selectedIds: number[], propertyName: string) => {
    ...
    if (targetLayer) {
      ...
      // FIX: Cast targetLayer to VectorLayer before using setStyle
      (targetLayer as VectorLayer<any>).setStyle((feature: any) => {
        switch(layerType) {
  ...
  const zoomToSelectedFeatures = useCallback((layerType: string, selectedIds: number[], propertyName: string) => {
    ...
    if (targetLayer) {
      ...
      if (source) {
        // FIX: Cast source to VectorSource before using getFeatures
        const features = (source as VectorSource<any>).getFeatures();
        ...
        if (selectedFeatures.length > 0) {
          const extent = selectedFeatures.reduce((ext: any, feature: any) => {
            ...
            if (geometry) {
              return extend(ext, geometry.getExtent());
            }
            return ext;
          }, createEmpty());
