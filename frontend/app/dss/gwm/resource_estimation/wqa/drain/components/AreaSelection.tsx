...
const AreaSelection: React.FC<AreaSelectionProps> = ({ onAreaConfirmed }) => {
  ...
  const [showVillageManager, setShowVillageManager] = useState<boolean>(false);
  ...
  // Helper function to handle village checkbox change
  const handleVillageCheckboxChange = (villageCode: number, checked: boolean): void => {
    if (checked) {
      // Add village to selection
      const newSelection = [...selectedVillages, villageCode];
      setSelectedVillages(newSelection);
    } else {
      // Remove village from selection
      const newSelection = selectedVillages.filter((code: number) => code !== villageCode);
      setSelectedVillages(newSelection);
    }
  };
              ...
              <div className="mt-3 p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                ...
                <div className="mb-3">
                  <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setVillageSearchTerm(e.target.value)}
                ...
                <div className="max-h-48 overflow-y-auto">
                  <div className="grid grid-cols-2 gap-2">
                    {villages
                      ...
                      .map((village) => {
                        const villageCode = Number(village.code);
                        return (
                          <label
                            key={village.code}
                          ...
                          >
                            <input
                              type="checkbox"
                              checked={selectedVillages.includes(villageCode)}
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                                handleVillageCheckboxChange(villageCode, e.target.checked);
                              }}
