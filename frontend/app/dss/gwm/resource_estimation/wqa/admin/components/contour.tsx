...
import { useGroundwaterContour } from '@/contexts/water_quality_assesment/admin/ContourContext';
 ...
 const villageAnalysisRef = useRef<HTMLDivElement>(document.createElement('div'));
  ...
  return (
    <div className="space-y-8 bg-gradient-to-br from-gray-50 to-white min-h-screen p-6">
      <FullScreenLoadingOverlay isVisible={isFullScreenLoading} message={generationProgress} />
      
      <div className={isFullScreenLoading ? "pointer-events-none opacity-50" : ""}>
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-8 rounded-3xl border-0 shadow-2xl text-white">
            ...
            {waterQualityParameters.map((parameter) => (
              ...
              >
                <input
                  ...
                  disabled={!yearSelected}
                  className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 w-5 h-5 disabled:cursor-not-allowed"
                />
                <span className={`text-sm font-medium capitalize ${
                  !yearSelected
                    ? 'text-gray-400'
                    : selectedCategories[parameter] ? 'text-blue-700' : 'text-gray-700'
                }`}>
                  {parameter.replace('_', ' ')}
                </span>
              </label>
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-8 rounded-2xl border-2 border-green-200 shadow-xl">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            {/* GENERATE GWQI BUTTON - HIDDEN AFTER SUCCESS */}
            {(!isGwqiGenerated || parametersChanged) && (
              <button
                onClick={handleGenerateGWQI}
              ...
              >
                {isGenerating ? (
                  <div className="flex items-center gap-3">
                    ...
                    Generating GWQI for {selectedYear}...
                  </div>
                ) : (
                  <div className="flex items-center gap-3">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    Generate GWQI {selectedYear ? `for ${selectedYear}` : ''}
                  </div>
                )}
              </button>
            )}
        ...
        {/* RESULTS SECTION - WITHOUT GWQI ANALYSIS RESULTS CARD */}
        {gwqiData && selectedYear && (
          <div className="space-y-8">
            <div className="bg-gradient-to-r from-green-500 to-emerald-500 p-6 rounded-2xl shadow-xl">
