...
const WellSelection: React.FC<WellSelectionProps> = ({ onWellsConfirmed, onReset }) => {
  ...
  useEffect(() => {
    if (wellSelectionMode === 'existing_and_new' && selectedSubDistricts.length > 0 && areaConfirmed && selectedYear) {
      console.log(`Fetching wells data for year ${selectedYear}`);
      fetchWellsData(selectedSubDistricts, selectedYear);
    }
  }, [wellSelectionMode, selectedSubDistricts, areaConfirmed, selectedYear]);

  useEffect(() => {
    if (yearSelected && selectedYear && !wellSelectionMode && !isWellTableSaved) {
      console.log("Auto-selecting existing wells mode after year selection");
  ...
  useEffect(() => {
    if (areaConfirmed && selectedSubDistricts.length > 0) {
      // Get bounds after a short delay to ensure layers are loaded
      const timer = setTimeout(() => {
        const bounds = getSelectedAreaBounds();
        if (bounds) {
          setSelectedRegionBounds(bounds);
          console.log('[BOUNDS] Region bounds set:', bounds);
        }
      }, 1500);
  ...
  }, [areaConfirmed, selectedSubDistricts, getSelectedAreaBounds]);
    ...
    const wellPoints = wellsData.map((well, index) => {
      ...
      if (!isNaN(latitude) && !isNaN(longitude) &&
        ...
        return {
          ...
          latitude,
          longitude,
          locationCode: String(well.Location || `WELL_${index + 1}`),
          subDistrict: String(well.SUB_DISTRI || 'Unknown'),
          properties: well
        };
      }
      return null;
  ...
  const handleWellAddFromMap = (wellData: WellData, coordinates: [number, number]) => {
    ...
    console.log(`Well added from map at ${coordinates[1]}, ${coordinates[0]}`);
  };
  ...
  const toggleMapAddMode = () => {
    ...
    if (!areaConfirmed || isWellTableSaved || !selectedYear) {
      console.log("Cannot toggle map add mode");
      return;
    }
    ...
    if (isMapAddModeEnabled) {
      ...
      console.log("Popup columns for map add:", popupColumns);
  ...
  const handleManualFormSubmit = () => {
    ...
    console.log("Adding manual well row:", manualFormData);
  ...
  const handleChangeCSV = () => {
    console.log("Changing CSV file - clearing current data");
    setWellsData([]);
  ...
  const handleSaveAndConfirm = async (): Promise<void> => {
    if (!areaConfirmed || !selectedYear) {
      console.log("Cannot proceed: Area not confirmed or year not selected");
      alert("Please confirm area and select year first");
      return;
    }

    setIsConfirming(true);
    console.log("Starting Save & Confirm process...");

    try {
      if (!isWellTableSaved) {
        console.log("Saving wells table...");
        const saved = saveWellTable();
        if (!saved) {
          throw new Error("Failed to save wells table");
        }
        await new Promise(resolve => setTimeout(resolve, 200));
      }

      console.log("Confirming final selections and posting to backend...");
      ...
      console.log("Save & Confirm completed successfully!");
    } catch (error: any) {
     console.log("Error during Save & Confirm:", error);
  ...
  const downloadCSVTemplate = () => {
    ...
    link.href = url;
    link.download = `wells_data_template_${selectedYear}.csv`;
              ...
              <div className="mt-4 p-3 bg-green-100 border-l-4 border-green-500 rounded">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-green-800 font-semibold">
                    Year {selectedYear} selected for analysis
                  </span>
                </div>
              </div>
            )}
              ...
              {wellSelectionMode === 'existing_and_new' && yearSelected && (
                <div className="p-3 bg-green-50 border border-green-200 rounded">
                  <p className="text-green-800 text-sm">
                    ...
                    Wells data from the selected subdistricts and year will be displayed in the table below.
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="flex-1 mt-4 p-4 bg-white rounded-lg shadow-md overflow-hidden flex flex-col">
          <div className="flex justify-between items-center mb-4 flex-shrink-0">
            <h4 className="text-lg font-medium text-gray-800">
              ...
              {existingWellsModified && !isWellTableSaved && (
          ...
          <div className="bg-yellow-50 p-3 rounded border border-yellow-200 mb-4 flex-shrink-0">
            <p className="text-sm text-yellow-800">
              ...
              {wellSelectionMode === 'existing_and_new'
                ? existingWellsModified
                  ? ` These are modified existing wells from year ${selectedYear}`
                  : ` These are existing wells from year ${selectedYear} in the selected area. You can edit the data and add rows/columns if needed.`
          ...
          >
            ...
            <div className="bg-gradient-to-r from-blue-500 to-indigo-600 px-6 py-5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-white">Add New Well</h3>
                  <p className="text-sm text-blue-100">Fill in the well details below</p>
                </div>
              </div>
              <button
                onClick={handleManualFormCancel}
            ...
            <div className="px-6 py-4 bg-blue-50 border-b border-blue-200">
              <div className="flex items-start gap-3">
                ...
                <p className="text-sm text-blue-800">
                  <strong className="font-semibold">Required fields:</strong> Latitude and Longitude must be filled with valid coordinates within the selected region.
                </p>
              </div>
            </div>
            ...
            {selectedRegionBounds && (
              <div className="px-6 py-3 bg-gradient-to-r from-indigo-50 to-purple-50 border-b border-indigo-200">
                <div className="flex items-center gap-2 text-sm">
                  <svg className="w-4 h-4 text-indigo-600" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" />
                  </svg>
                  <span className="font-semibold text-indigo-900">Valid Region:</span>
                  ...
                  return (
                    <div key={column} className={isYearField ? 'md:col-span-2' : ''}>
                      ...
                      <input
                        ...
                        placeholder={
                          column === 'Latitude' ? 'e.g., 28.7041' :
                          column === 'Longitude' ? 'e.g., 77.1025' :
                          isYearField ? selectedYear || '' :
                          `Enter ${getColumnDisplayName(column).toLowerCase()}`
                        }
            ...
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex gap-3">
              <button
                ...
                className="flex-1 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-200 transform hover:scale-[1.02] shadow-md hover:shadow-lg flex items-center justify-center gap-2"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                </svg>
