...
const WellSelection: React.FC<WellSelectionProps> = ({ onWellsConfirmed, onReset }) => {
  ...
  useEffect(() => {
    if (wellSelectionMode === 'existing_and_new' && selectedSubDistricts.length > 0 && areaConfirmed) {
      fetchWellsData(selectedSubDistricts);
    }
  }, [wellSelectionMode, selectedSubDistricts, areaConfirmed]);
  ...
  const toggleMapAddMode = () => {
    ...
    if (isMapAddModeEnabled) {
      ...
      // Enable mode - pass ALL display columns to popup
