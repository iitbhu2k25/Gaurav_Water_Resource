...
import { RechargeProvider } from "@/contexts/groundwater_assessment/admin/RechargeContext";
...
function ComputeAvailableWaterButton() {
  ...
  const handleClick = () => {
    // Build query parameters for location
    const params = new URLSearchParams();
    
    if (selectedState) {
      params.append('state', selectedState.toString());
    }
    if (selectedDistricts.length > 0) {
      params.append('districts', selectedDistricts.join(','));
    }
    if (selectedSubDistricts.length > 0) {
      params.append('subdistricts', selectedSubDistricts.join(','));
    }

    // Store stress data in sessionStorage (for large datasets)
    ...
    console.log('SubDistricts:', selectedSubDistricts);
    console.log('Stress Data Villages:', stressTableData.length);
      ...
      <div className="flex-grow overflow-y-auto bg-white mx-2 sm:mx-3 my-2 rounded-lg shadow-md">
        <div className="p-3 sm:p-6">
          {activeStep === 1 ? (
            ...
            <GroundwaterForecast
              activeTab="groundwater-forecast"
              step={activeStep}
            />
          ) : null}
        </div>
        ...
        <div className="w-full h-full relative">
          ...
          <div className="absolute top-0 left-0 right-0 z-10 bg-white border-b border-gray-200 px-2 sm:px-4 py-2 sm:py-3">
            <div className="flex items-center justify-between">
              <h3 className="text-sm sm:text-base font-medium text-gray-700">Interactive Map</h3>
              <button
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <div className="flex-shrink-0 bg-gray-100 p-2 sm:p-4 border-t border-gray-300 mx-2 sm:mx-3 mb-2 rounded-lg shadow-md">
        <div className="flex flex-col sm:flex-row justify-center items-center gap-2 sm:gap-4">
          ...
          {activeStep >= 3 && (
            <div className="w-full sm:w-auto flex flex-col sm:flex-row justify-center items-center gap-2 sm:gap-3 mt-3">
              <PDF />
