...
const Maping: React.FC = () => {
  ...
  // Well points layer
  useEffect(() => {
    console.log("well_points1", well_points);
    ...
    console.log("well_points2", well_points);
    if (!well_points || well_points.length === 0) {
      setFeatureCounts(prev => ({ ...prev, wells: 0 }));
      return;
    }
    console.log("well_points3", well_points);
    ...
    const wellSource = new VectorSource({ features: features });
  ...
  }, [riverLayer, riverFilter, layerVisibility.river, showTitles, defaultWorkspace, hasSelections]);

  useEffect(() => {
    handleVectorLayer(stretchLayer, stretchLayerRef, "stretch", 5, layerVisibility.stretch, stretchFilter as any);
  }, [stretchLayer, stretchFilter, layerVisibility.stretch, showTitles, defaultWorkspace, hasSelections]);

  useEffect(() => {
    handleVectorLayer(drainLayer, drainLayerRef, "drain", 6, layerVisibility.drain, drainFilter as any);
  }, [drainLayer, drainFilter, layerVisibility.drain, showTitles, defaultWorkspace, hasSelections]);

  useEffect(() => {
    handleVectorLayer(catchmentLayer, catchmentLayerRef, "catchment", 7, layerVisibility.catchment, catchmentFilter as any);
  }, [catchmentLayer, catchmentFilter, layerVisibility.catchment, showTitles, defaultWorkspace, hasSelections]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const wmsSource = new ImageWMS({
        url: layerUrl,
        params: { LAYERS: fullLayerName, TILED: true, FORMAT: "image/png", TRANSPARENT: true },
        ratio: 1,
  ...
  const handleClick = () => {
    ...
    if (selectedDrains.length > 0) setShowCatchment(true);
  };
  ...
  return (
    <div className="relative w-full h-[600px] flex flex-col bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="relative w-full h-full flex-grow overflow-hidden rounded-xl shadow-2xl border border-gray-200" ref={containerRef}>
        ...
        <div className="hidden md:block"><GISCompass /></div>
        ...
        <div className="absolute top-3 left-1/2 transform -translate-x-1/2 z-40 bg-white/95 backdrop-blur-md rounded-2xl shadow-xl px-6 py-3 flex items-center space-x-4">
          ...
          <div className="flex space-x-2">
            {["layers", "basemap", "tools"].map((panel) => (
              <button key={panel} onClick={() => togglePanel(panel)} className={`p-2.5 rounded-full transition-all duration-200 hover:scale-110 ${activePanel === panel ? "bg-blue-100 text-blue-600" : "hover:bg-gray-100 text-gray-700"}`}>
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={panel === "layers" ? "M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" : panel === "basemap" ? "M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2h2a2 2 0 002-2v-1a2 2 0 012-2h1.945M5.05 9h13.9c.976 0 1.31-1.293.455-1.832L12 2 4.595 7.168C3.74 7.707 4.075 9 5.05 9z" : "M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"} />
                </svg>
              </button>
            ))}
            <button onClick={toggleFullScreen} className="p-2.5 rounded-full hover:bg-gray-100 text-gray-700">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={!isFullScreen ? "M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5v-4m0 4h-4m4 0l-5-5" : "M6 18L18 6M6 6l12 12"} />
              </svg>
            </button>
          </div>
        </div>
          ...
          <div className="absolute top-20 left-1/2 transform -translate-x-1/2 z-30 bg-white/95 backdrop-blur-md rounded-xl shadow-2xl p-6 max-w-md w-full mx-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              {Object.entries(baseMaps).map(([key, baseMap]) => (
                <button key={key} onClick={() => changeBaseMap(key)} className={`flex flex-col items-center p-4 rounded-xl transition-all duration-200 border-2 ${selectedBaseMap === key ? "bg-blue-100 border-blue-300 text-blue-700" : "bg-gray-50 hover:bg-gray-100 border-gray-200"}`}>
            ...
            <div className="space-y-3">
              ...
              {rasterLayerInfo && (
                <div className="p-4 rounded-xl bg-gradient-to-r from-purple-50 to-violet-50 border border-purple-200">
                  ...
                  <input type="range" min="5" max="95" step={10} value={layerOpacity} onChange={handleOpacityChange} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                </div>
              )}
            </div>
          </div>
